...
import { getInventoryReport } from "@/services/reports/inventoryReport.service";
...
export async function getInventoryReportHandler(req: Request, res: Response) {
  ...
  try {
    ...
    logger.error(`[getInventoryReport] ${err?.message ?? error}`);
    return res.status(500).json({ error: "Unable to generate inventory report" });
  }
}
