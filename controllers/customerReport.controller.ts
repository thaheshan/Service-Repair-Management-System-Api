...
import { getCustomerReport } from "@/services/reports/customerReport.service";
...
export async function getCustomersReport(req: Request, res: Response) {
  ...
  try {
    ...
    logger.error(`[getCustomersReport] ${err?.message ?? error}`);
    return res.status(500).json({ error: "Unable to generate customer report" });
  }
}
